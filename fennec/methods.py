...
class GBC(TransferabilityMethod):
    # https://arxiv.org/abs/2111.12780  
    # https://github.com/google-research/google-research/blob/master/stable_transfer/transferability/gbc.py
...
class RawHSIC(TransferabilityMethod):
    def __init__(self, reference_architecture:str=None, n_dims:int=None):
        print('raw HSIC')
        self.reference_architecture = reference_architecture
        self.n_dims = n_dims
    ...
    def forward(self):
        ...
        res, _ = self.hsic_gam(self.features, reference_features)
    ...
    def hsic_gam(self, X, Y, alph = 0.5):
        """
...
class KITE_HSIC(TransferabilityMethod):
    # not working!
    def __init__(self, reference_architecture:str=None, n_dims:int=None):
        self.n_dims = n_dims
        self.reference_architecture = reference_architecture
    ...
    def gram_linear(self, x):
        # compute kernal matrix for a linear kernel
        return x.dot(x.T)

    def gram_rbf(self, x, threshold=0.8):
    ...
    def forward(self):
        ...
        gram_feature = self.gram_linear(self.features)
        gram_reference = self.gram_linear(reference_features)

        score = self.cka(gram_feature, gram_reference)
...
class LDA(TransferabilityMethod):
    def __init__(self, n_dims=None) -> None:
        self.n_dims = n_dims
