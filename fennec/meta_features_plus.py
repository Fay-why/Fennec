...
from model_sim_RSA import FDA_score
from feature_extractor import get_forward, demo_from_homespace, load_model_name, ok_299, ok_32
# from .archi import get_archi
from archi_graph import get_archi_imgclob
...
def model_meta(architecture, source_dataset, idx=None, num=None):
    # __import__('IPython').embed()
    model = demo_from_homespace(model_name=architecture)
    model.eval()
    res = []
    ...
    archi_type = get_archi_imgclob(architecture) # new

    res.extend(archi_type) # new



    print(f'{architecture}: {res}')
    return res
...
model_meta_res = {}
dataset_meta_res = {}
models_name = load_model_name()
for idx, m in enumerate(models_name):
    # archi, source = m.split('%')
    model_meta_res[m] = model_meta(architecture=m, source_dataset=None, idx=idx, num=len(models_name))
...
def build_full_performance_matrix(result_file='./oracles/large.csv'):

    # build zero matrix

    zero_matrix = np.zeros((105, len(all_datasets)), dtype=np.float64)
...
out_file = f'./results/test_cold.csv'
...
def test(predict_dataset, args, run=0):
    ...
    # 3.mask specific dataset, do the matrix factorization, get dataset and model embeddings via matrix factorization
    ...
    # 4.get the new dataset vector using regression <forward features on resnet50, historical dataset embeddings>, do the vector multiply
    model_vector = U
    dataset_vector = V
    new_dataset = np.zeros((1, k))

    # 4.1 get the forward features of historical datasets on 
    ...
    for i, model_name in enumerate(models_name):
        tmp =  model_meta_res[model_name] + dataset_meta_res[predict_dataset]
...
if __name__ == "__main__": 
    ...
    parser.add_argument('--no_completion_rebuilding', type=str,
                        default='no', help='do not use completion, using [RSA or SFDA] directly')
