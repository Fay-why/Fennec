...
def add_plasticity(results:pd.DataFrame, methods:list, weight:float=1) -> pd.DataFrame:
	...
	for idx, row in results.iterrows():
		...
		for method in methods:
			...
			results.at[idx, method] = val + weight * (layers / 357)
	
	return results

def add_meta(results:pd.DataFrame, methods:list, weight:float=1) -> pd.DataFrame:
	stats = {}
	for method in methods:
		col = results
...
class MetricEval:
	...
	def __init__(self, path:str, oracle_path:str='./oracles/large.csv'):
	...
	def aggregate(self, constants:list=['Target Dataset'], methods:list=None, variance_over:str='Run', metric=pearson, aggregate:bool=True):
		...
		for run in self.results[variance_over].unique():  # run in [0,1,2,3,4]
			...
			if aggregate:
				...
				all_runs.append(aggregated_run_metrics)  # all_runs = [{aggregated[NCE]=score....}, {}, {},]
			else:
		...
		# Compute variance on different runs
		if aggregate:
			run_data = {method: [run[method] for run in all_runs] for method in methods}  # {NCE:score@0, score@1...}
...
if __name__ == '__main__':
	...
	for method in mean:
		...
		# print(f'{method:20s}: {mean[method]:6.2f}% +/- {np.sqrt(var[method]):4.2f} ')
