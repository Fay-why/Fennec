...
from datasets import BalancedClassSampler, FixedBudgetSampler, FullDatasetSampler
import utils
...
class FullDatasetExperimentParams:
    """ using full dataset samples"""
    def __init__(self) -> None:
        self.experiment_name = f'fixed_budget_full'

    def create_dataloader(self, dataset:str, batch_size:int, **kwdargs):
        return FullDatasetSampler(dataset, batch_size=128, **kwdargs)
...
class Experiment:
    ...
    def __init__(self, methods:Dict[str, TransferabilityMethod], budget:int=500, runs:int=5, probe_only:bool=False, model_bank:str='controlled', append:bool=False, name:str=None, full=False):
        if full:
            self.params = FullDatasetExperimentParams()
        else:
    ...
    def cur_cache_path(self):
        if self.full:
            return self.cache_path(self.architecture, self.source_dataset, self.target_dataset, 0)
    ...
    def probe(self):
        ...
        with torch.no_grad():
            ...
            for x, y in dataloader:
                ...
                all_feats.append(torch.cat([x for x in self.model._extracted_feats], dim=0))
    ...
    def evaluate(self):
        # return a row of result with many eval methods
        params = self.probe()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--budget'    , help='Number of image in the probe set. Default is 500.', default=1000, type=int)
    parser.add_argument('--runs'      , help='Number of probe sets sampled per transfer. Default is 5.', default=5, type=int)
    parser.add_argument('--probe_only', help='Set this flag if you only want to generate probe sets.', default=True, action='store_true')
    ...
    exp = Experiment(my_methods, args.budget, runs=args.runs, probe_only=args.probe_only, model_bank=args.model_bank).run()
    exp.run()
