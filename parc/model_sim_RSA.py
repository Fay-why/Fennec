...
class Cluster(Model_Sim_RSA):
    ...
    def without_kmeans(self, features, true_labels, pca_dim=32):
        ...
        for idx, model_name in enumerate(self.models_name):
            ...
            res[idx] = metrics.silhouette_score(feat, label, metric='euclidean') # 58.03%
            #res[idx] = metrics.calinski_harabasz_score(feat, label)    # 54.65%
            #res[idx] = metrics.davies_bouldin_score(feat, label)   # -54.89%
    ...
    def with_kmeans(self, features, true_labels, pca_dim=32):
        ...
        for idx, model_name in enumerate(self.models_name):
            ...
            res[idx] = metrics.adjusted_mutual_info_score(true_label, pred_label) # 57.06% / 56.00
            #res[idx] = metrics.adjusted_rand_score(true_label, pred_label)  # 56.4%
            #res[idx] = metrics.v_measure_score(true_label, pred_label)  # 56.73%
            #res[idx] = metrics.homogeneity_completeness_v_measure(true_label, pred_label) # error
            #res[idx] = metrics.fowlkes_mallows_score(true_label, pred_label) # 56.46%
