...
class NABird(Dataset):
	...
	def _load_metadata(self):
		images = pd.read_csv(os.path.join(self.root, 'images.txt'), sep=' ',
							 names=['img_id', 'filepath'])
		image_class_labels = pd.read_csv(os.path.join(os.path.join(self.root, 'image_class_labels.txt')),
										 sep=' ', names=['img_id', 'target'])
...
class DatasetCache(torch.utils.data.Dataset):
	...
	def cache_path(self, idx:int) -> str:
		return f'{path}/cache/datasets/{self.dataset}/{"train" if self.train else "test"}_{idx}.npy'
		
	def glob_path(self) -> str:
		return f'{path}/cache/datasets/{self.dataset}/{"train" if self.train else "test"}_*'
...
class FullDatasetSampler(torch.utils.data.DataLoader):
	def __init__(self, dataset:str, batch_size:int, train:bool=True, min_instances_per_class:int=2, **kwdargs):
		num_classes = constants.num_classes[dataset]
		dataset_obj = DatasetCache(dataset, train)
		map_cache = ClassMapCache(dataset, train)
		...
		# 	super().__init__(dataset_obj, batch_size, sampler=samples[:probe_size], **kwdargs)
		# 	return
		super().__init__(dataset_obj, batch_size, **kwdargs)



if __name__ == '__main__':
	#FixedBudgetSampler('voc2007', 128, 500, train=True)
	#FixedBudgetSampler('nabird', 128, 100, train=True)
	FullDatasetSampler('cifar10', 128, train=True)
