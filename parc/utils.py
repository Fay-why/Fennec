...
def get_source_model_path(architecture:str, source_dataset:str) -> str:
	return f'{path}/models/{architecture}/{architecture}_{source_dataset}.pth'

def get_transfer_model_path(architecture:str, source_dataset:str, target_dataset:str) -> str:
	return f'{path}/models/{architecture}/{architecture}_{target_dataset}_from_{source_dataset}.pth'
...
def load_source_model(architecture:str, source_dataset:str) -> torch.nn.DataParallel:
	...
	if source_dataset in constants.num_classes:
		num_classes = constants.num_classes[source_dataset]
	else:
		num_classes = 1000

	net = torch.nn.DataParallel(getattr(torchvision.models, architecture)(pretrained=False, num_classes=num_classes, **kwdargs))
	net.load_state_dict(torch.load(get_source_model_path(architecture, source_dataset), map_location=torch.device('cpu')))
	return net
...
def load_transfer_model(architecture:str, source_dataset:str, target_dataset:str) -> torch.nn.DataParallel:
	...
	net = torch.nn.DataParallel(getattr(torchvision.models, architecture)(pretrained=False, num_classes=constants.num_classes[target_dataset], **kwdargs))
